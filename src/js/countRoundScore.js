...
function determineWinner(user1, user2) {
    ...
    if (user1.total > user2.total) {
        victor = user1;
        user1.victoryCount++;
        updateUserSingleProperty('victoryCount', user1.victoryCount, JSON.parse(localStorage.getItem('index')));
        console.log('user victory', user1.victoryCount);
        ...
        console.log('DrAw...', user1.victoryCount, user2.victoryCount);
    }
...
function showRoundEndModal(user1, user2, victor, isDraw) {
    ...
    if(isDraw) {
        user2VictoryCount.style.backgroundImage = user1VictoryCount.style.backgroundImage;
    } else {
