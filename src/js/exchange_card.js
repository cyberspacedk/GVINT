...
export function threeRandomCardAfterRaund(obj) {
    let i = 0;
    while (i < 3 && obj.cardHand.length < 10) {
        const index = Math.floor(Math.random() * (obj.deck.length))
        const randomCard = obj.deck[index]
        obj.deck.splice(`${index }`, 1)
